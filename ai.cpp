...
void AI::LogMinimaxMove(bool is_maximizing, int depth,
                        const std::string& message, std::pair<int, int> move) const {
    std::string indent(depth * 2, ' ');
    std::string player = is_maximizing ? "MAX" : "MIN";
    if (move.first != -1 && move.second != -1) {
        cout << indent << player << ": " << message << " ("
             << move.first << "," << move.second << ") at depth "
             << depth << endl;
    } else {
        cout << indent << player << ": " << message
             << " at depth " << depth << endl;
...
int AI::Minimax(Board board, bool is_maximizing, char ai_player,
                char human_player, int depth, int alpha, int beta,
                int max_depth) {
...
string AI::GenerateExplanation(const Board& board_before,
                              const Board& board_after,
                              char ai_player, char human_player) {
    ...
    for (const auto& move : opponent_moves) {
        ...
        if (EvaluateBoard(temp, ai_player, human_player) == -10) {
            if (board_after[move.first][move.second] == ai_player) {
                return "Blocked opponent's win at (" +
                       to_string(move.first) + ", " +
...
pair<int, int> AI::GetBestMove(const Board& board, char ai_player) {
    ...
    if (difficulty_ == "easy") {
        ...
        audit_.LogMove(chosen.first, chosen.second, 0, 0,
                       "Random move (easy mode)", 0);
        return chosen;
    }
    int best_score = std::numeric_limits<int>::min();
    pair<int, int> best_move = {-1, -1};
    int max_depth = (difficulty_ == "medium") ? 2 : 6;
    for (const auto& move : legal_moves) {
        Board temp = board;
        temp[move.first][move.second] = ai_player;
        int score = Minimax(temp, false, ai_player, human_player,
                            1, -1000, 1000, max_depth);
        if (score > best_score) {
            best_score = score;
            best_move = move;
        }
...
void AIAudit::LogMove(int row, int col, int score, int depth,
                      const string& explanation, int move_number) {
